...
# преобразовывает изображение, выделяя на нём квадратом лицо
def face_rec():
    # подгружаем изображение
    gal_face_img = face_recognition.load_image_file("img/gal1.jpg")
    # получаем в кортеже координаты лиц на фото
...
def extracting_faces(img_path):
    ...
    for face_location in faces_locations:
        ...
        # получили объект лица
        pil_img = Image.fromarray(face_img)
        pil_img.save(f"img/faces/{count}_face_img.jpg")
        count += 1

    return f"Found {count} face(s) in this photo"


# сравнивает лица с изображений
